...
from cadilib.ionogramparser.cadioutput import CADIheader

def common_assertions(output):
    assert type(output) == CADIdata
    ...
    assert type(metadata) == CADIheader
...
class TestCADIRaw:
    def test_read_rawmd4(self, mock_raw_file, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file)
    ...
    def test_read_rawmd4_incomplete_header(self, mock_raw_file_incomplete_header, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file_incomplete_header)
        common_assertions(output)
        metadata = output.metadata
        assert metadata.incompleteheader == True
        assert metadata.incompletedata == True
    
    def test_read_rawmd4_incomplete_header_2(self, mock_raw_file_incomplete_header_2, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file_incomplete_header_2)
        common_assertions(output)
        metadata = output.metadata
        assert metadata.incompleteheader == True
        assert metadata.incompletedata == True
    ...
    def test_read_rawmd4_incomplete_data(self, mock_raw_file_incomplete_data, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file_incomplete_data)
        common_assertions(output)
        metadata = output.metadata
        assert metadata.incompletedata == True
    ...
    def test_read_rawmd4_incomplete_data_2(self, mock_raw_file_incomplete_data_2, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file_incomplete_data_2)
        common_assertions(output)
        metadata = output.metadata
        assert metadata.incompletedata == True
        assert metadata.incompleteheader == False
    ...
    def test_read_rawmd4_incomplete_data_3(self, mock_raw_file_incomplete_data_3, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file_incomplete_data_3)
        common_assertions(output)
        metadata = output.metadata
        assert metadata.incompletedata == True
        assert metadata.incompleteheader == False

    def test_read_rawmd4_incomplete_data_4(self, mock_raw_file_incomplete_data_4, test_raw_reader):
        output = test_raw_reader.read_raw_data(mock_raw_file_incomplete_data_4)
        common_assertions(output)
        metadata = output.metadata
        assert metadata.incompletedata == True
        assert metadata.incompleteheader == False
